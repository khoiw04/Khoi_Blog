...
const embedHandlers = {
  ...
  link: (node) => {
    ...
    return `
      <div class="link-card-wrapper button">
  ...
  x: (node) => {
    ...
    <figure class="w-full my-4 mx-auto button">
      <blockquote class="flex max-w-xl w-full my-2" data-dnt="true" id="${uniqueId}">
        <a href="${twitterUrl}"></a>
      </blockquote>
    </figure>
    `
  ...
  github: (node) => {
    ...
    return `
    <a href="https://github.com/${repo}" class="block button rounded px-2 py-4 transition-colors my-4" target="_blank" rel="noopener noreferrer" data-repo="${repo}">
        <div class="flex items-center gap-3 mb-3">
          <div class="gc-owner-avatar" style="background-size: cover; background-position: center;" aria-hidden="true"></div>
          <span class="size-6 rounded-full shrink-0">
            <span>${owner}<span class="mx-2" aria-hidden="true">/</span><strong>${name}</strong></span>
          </span>
          <svg class="size-6 shirnk-0" width="24" height="24" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
        ...
        <p class="opacity-60 my-3">--</p>
        <div class="flex items-center opacity-60 gap-3">
          <svg class="size-4" height="16" width="16" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
