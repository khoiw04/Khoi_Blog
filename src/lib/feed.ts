...
async function fixRelativeImagePaths(
  ...
  for (const img of imageTags) {
    ...
    if (src.startsWith('./') || src.startsWith('../')) {
      // Build path relative to /src/content/blog
      let resolvedPath: string
      if (src.startsWith('./')) {
        // ./xxx -> postDir/xxx
        resolvedPath = path.posix.join('/src/content/blog', postDir, src.slice(2))
      } else {
...
async function generateFeedInstance(context: APIContext) {
  ...
  const feed = new Feed({
    ...
    updated: new Date(),
    generator: 'KhoiWN04 Feed Generator',
    feedLinks: {
      rss: `${siteUrl}/rss.xml`,
  ...
  const posts = await getCollection(
    'blog',
    ({ id }: CollectionEntry<'blog'>) => !id.startsWith('_')
  )
